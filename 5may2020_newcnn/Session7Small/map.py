...
class Game(Widget):
    ...
    def get_obs(self, xx, yy):
        area = (self.car.x-60,self.car.y-60,self.car.x+60,self.car.y+60)
        iobj = img.rotate(90,expand=True).crop(area)
        #iobj.save("xxafter60" + str(i) +".png")
        img2 = PILImage.open("./images/car.png").resize((20,10))
        iobj.paste(img2.rotate(Vector(*self.car.velocity).angle((xx,yy)),expand=True),(50,45))
        #iobj.save("xxafter120" + str(i) +".png")
        iobj.thumbnail((28,28))
        #iobj.save("xxafter28" + str(i) +".png")
    ...
    def step(self, action, last_distance):
        ...
        if sand[int(self.car.x),int(self.car.y)] > 0:
            ...
            print(1, goal_x, goal_y, distance, max(int(self.car.x),0),max(int(self.car.y),0), im.read_pixel(max(int(self.car.x),0),max(int(self.car.y),0)))
            #Penalty for going on sand
            reward = -5
        else: # otherwise
            self.car.velocity = Vector(2, 0).rotate(self.car.angle)
            reward = 5
            print(0, goal_x, goal_y, distance, max(int(self.car.x),0),max(int(self.car.y),0), im.read_pixel(max(int(self.car.x),0),max(int(self.car.y),0)))
